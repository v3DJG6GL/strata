...
| `STRATA_SCAN_PRECOUNT`     | *(none)*                       | How the live progress % is computed. Default: estimate the total from the previous scan's file count (instant, no extra I/O; the first-ever scan shows an indeterminate bar). If truthy, pre-count every file on each scan for an exact %, at the cost of a full extra metadata pass — slow on large spinning-disk trees. |
