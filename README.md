...
| `STRATA_SCAN_PRECOUNT`     | `false`                        | How the live progress % is computed. `false` estimates the total from the previous scan's file count (instant, no extra I/O; the first-ever scan shows an indeterminate bar). `true` pre-counts every file on each scan for an exact %, at the cost of a full extra metadata pass — slow on large spinning-disk trees. |
| `STRATA_KEEP_SNAPSHOTS`    | `30`                           | Number of snapshots to retain. |
| `STRATA_FILE_TOP`          | `12`                           | How many of the biggest individual files per directory to surface as their own chart slices / in Compare (`0` = none). Directory totals and file counts are always exact; this only tunes which files are shown individually. |
| `STRATA_FILE_FLOOR`        | *(off)*                        | Also store **every** file at least this big as its own slice — human-readable (`512K`, `100M`, `1G`; `0` = every file = "DUC parity"; empty = off). Combines with `STRATA_FILE_TOP` (union: a file is kept if it is among the top-N **or** ≥ the floor). A very low/`0` floor on a tree with millions of files costs roughly +250 MB indexer RAM and +75 MB snapshot per 5M files; the browsable list is capped at 50,000 files/folder. The chart/Compare serve at most ~6 files/folder at the top level and ~500 on drill-in (the file count stays exact). |
